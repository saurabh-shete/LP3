...
contract SimpleBank
{
    ...
    { // modifier can check wheather code is
...
{ // check upto to all client store in array
    if (clients[i].client_address == msg.sender)
    {                    // now check client address matched with sender only that client intiate transaction
...
clientCounter = 0; // those client join contract assign there
    ...
    {                              // setManager method will be used to set the manager address to
                                   // variables
...
// contract.
clients.push(client_account(clientCounter++, msg.sender,address(msg.sender).balance)); // push() array method to add items into
                                                                    // a storage array.
